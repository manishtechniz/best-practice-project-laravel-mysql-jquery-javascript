    ...
    <details open>
        ...
        <ul>
            @foreach ($laravelQuestions as $list)
                @if (! auth()->user()->can('php:'.$list['sort'])) @continue @endif
                ...
                <li>
                    @if (empty($list['question']))
                        ...
                        {!! $list['closure']() !!}
                    @else
                        <a style="font-weight:bold" href="{{ route('question', ['question' => $list['question'], 'permission' => 'php:'.$list['sort']]) }}" >
                            {{ $list['sort'] }}. {{ $list['label'] }}
                        </a>

                        @if(! empty($allQuestions)) @dump($allQuestions[$list['question']]) @endif
                    @endif
                </li>
            @endforeach
        </ul>
